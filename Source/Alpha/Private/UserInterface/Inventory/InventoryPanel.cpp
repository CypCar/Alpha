...
void UInventoryPanel::LinkToInventory(UInventoryComponent* InputInventory, AAlphaCharacter* InputCharacter)
{
    ...
    if (InputCharacter)
    {
        ...
        {
            UE_LOG(LogTemp, Error, TEXT("%s: InventorySubMenuClass was null!"), *FString(__FUNCTION__));
        }
    }
    ...
    if (InputInventory)
    {
        ...
        else
        {
            UE_LOG(LogTemp, Warning,
                TEXT("%s: Inventory %s is already linked to inventory: %s"),
                *FString(__FUNCTION__),
                *GetName(),
...
void UInventoryPanel::UnlinkFromInventory()
{
    ...
    {
        // Remove all functions from the delegate's invocation list that are bound to the specified UserObject
        const uint8 DelegatesRemoved = InventoryReference->InventoryWasUpdated.RemoveAll(this);
        if (DelegatesRemoved > 0)
        {
            UE_LOG(LogTemp, Warning, TEXT("%s: %d InventoryWasUpdated delegates unbound from %s."),
    ...
    if (ItemDragDrop->SourceItem && InventoryReference)
    {
        ...
        if (ItemDragDrop->SourceItem->GetOwningInventory() != InventoryReference)
        {
            ...
            case EItemAddResult::IAR_AllItemsAdded:
                ItemDragDrop->SourceItem->GetOwningInventory()->HandleRemoveItem(
                    ItemDragDrop->SourceItem, AddResult.AmountAdded);
                break;
            }
